...
const ust2 = async (bot, chat_id, text, lang) => {
  let message, kbb

  await updateUser({telegram_id: chat_id}, {name: text, step: 4})

  const user = await getUser({telegram_id: chat_id}), bio = report(user, 'USER', lang)
...
const ust4 = async (bot, chat_id, text, lang) => {
  let message, kbb

  await updateUser({telegram_id: chat_id}, {number: text, step: 4})
...
const ust6 = async (bot, chat_id, text, lang) => {
  let message, kbb

  await updateUser({telegram_id: chat_id}, {lang: text, step: 4})
...
const userSettings = async (bot, user, text, lang) => {
  try {
    if (text === kb.user.pages.uz.settings || text === kb.user.pages.ru.settings) await ust0(bot, user, lang)
      ...
      if (type === kb.user.settings.uz.name || type === kb.user.settings.ru.name) await ust2(bot, user.telegram_id, text, lang)
      else if (type === kb.user.settings.uz.number || type === kb.user.settings.ru.number) await ust4(bot, user.telegram_id, text, lang)
      // else if (type === kb.user.settings.uz.language || type === kb.user.settings.ru.language) await ust6(bot, user.telegram_id, text, lang)
    }
  } catch (e) {
    console.log(e)
  }
